...
def create_tables():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS item(id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100), category VARCHAR(50), min_bid_amt INT CHECK(min_bid_amt >0), organization_id BIGINT, FOREIGN KEY(organization_id) REFERENCES organization(reg_no),bid_start_time TIMESTAMP, bid_end_time TIMESTAMP)")
