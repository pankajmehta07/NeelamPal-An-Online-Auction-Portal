...
def create_tables():
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS item(id SERIAL PRIMARY KEY, name VARCHAR(100), category VARCHAR(50),description varchar(500), min_bid_amt INT CHECK(min_bid_amt >0), organization_id BIGINT, fileurl VARCHAR(255), FOREIGN KEY(organization_id) REFERENCES organization(reg_no),bid_start_time TIMESTAMP, bid_end_time TIMESTAMP, fileid VARCHAR(100))")
            cursor.execute("CREATE TABLE IF NOT EXISTS bid(id SERIAL PRIMARY KEY, created_at TIMESTAMP,item_id INT,FOREIGN KEY(item_id) REFERENCES item(id), bidder_id BIGINT, FOREIGN KEY(bidder_id) REFERENCES bidder(citizenship_no), amount INT NOT NULL)")
